...
export class init1578064736032 implements MigrationInterface {
    name = 'init1578064736032'

    public async up(queryRunner: QueryRunner): Promise<any> {
        await queryRunner.query("CREATE TABLE `user` (`id` int NOT NULL AUTO_INCREMENT, `created_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updated_at` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6), `services` text NOT NULL, `username` varchar(32) NOT NULL, `password` varchar(128) NOT NULL, `email` varchar(128) NOT NULL, `firstname` varchar(32) NOT NULL, `lastname` varchar(32) NOT NULL, `role` enum ('admin', 'user', 'ghost') NOT NULL DEFAULT 'ghost', `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6), `deletedAt` datetime NULL DEFAULT null, `avatarId` int NULL, UNIQUE INDEX `IDX_78a916df40e02a9deb1c4b75ed` (`username`), UNIQUE INDEX `IDX_e12875dfb3b1d92d7d7c5377e2` (`email`), UNIQUE INDEX `REL_58f5c71eaab331645112cf8cfa` (`avatarId`), PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        await queryRunner.query("CREATE TABLE `document` (`id` int NOT NULL AUTO_INCREMENT, `created_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updated_at` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6), `fieldname` varchar(255) ('picture', 'document', 'avatar') NOT NULL, `filename` varchar(255) NOT NULL, `path` varchar(255) NOT NULL, `mimetype` varchar(255) ('application/vnd.ms-excel', 'application/msword', 'application/zip', 'application/pdf', 'image/bmp', 'image/gif', 'image/jpeg', 'image/png', 'text/csv') NOT NULL, `size` int NOT NULL, `deletedAt` datetime NULL DEFAULT null, `userId` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        await queryRunner.query("CREATE TABLE `refresh_token` (`id` int NOT NULL AUTO_INCREMENT, `refreshToken` varchar(255) NOT NULL, `ip` varchar(45) NOT NULL, `pid` int NOT NULL DEFAULT 0, `expires` datetime NOT NULL, `userId` int NULL, UNIQUE INDEX `IDX_fd232a2ab74eb52217b050cfb1` (`userId`, `ip`, `pid`), PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
