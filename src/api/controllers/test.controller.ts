...
export class TestController extends BaseController {
   ...
   */
   public async get(req: Request, res : Response, next : Function) {
    try {
      ...
      const test = await repository.jsonAPI_findOne(req,req.params.id);
  ...
  public async create (req: Request, res : Response, next: Function) {
    try {
      ...
      const test = new Test(req.body.data.attributes);
  ...
  public async update (req: Request, res : Response, next: Function) {
    ...
    try {
      const repository = getRepository(Test);
      const test = await repository.findOne(req.params.id);
      repository.merge(test, req.body.data.attributes);
  ...
  public async list (req: Request, res : Response, next: Function) {
    ...
    try {
      const repository = getCustomRepository(TestRepository);
      const [tests,total] = await Promise.all([repository.jsonAPI_find(req),repository.count()]);
      res.json( new TestSerializer(req,total).serialize(tests)  );
    }catch(e) {
      next(e);
    }
  }
  ...
  public async remove (req: Request, res : Response, next: Function) {
    ...
    try {
      ...
      const test = await testRepository.findOne(req.params.id);
      const repository = getRepository(Test);
