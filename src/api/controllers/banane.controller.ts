...
export class BananeController extends BaseController {
   ...
   */
   public async get(req: Request, res : Response, next : Function) {
    try {
      ...
      const banane = await repository.jsonAPI_findOne(req,req.params.id);
  ...
  public async create (req: Request, res : Response, next: Function) {
    try {
      ...
      const banane = new Banane(req.body.data.attributes);
  ...
  public async update (req: Request, res : Response, next: Function) {
    ...
    try {
      const repository = getRepository(Banane);
      const banane = await repository.findOne(req.params.id);
      repository.merge(banane, req.body.data.attributes);
  ...
  public async list (req: Request, res : Response, next: Function) {
    ...
    try {
      const repository = getCustomRepository(BananeRepository);
      const [bananes,total] = await Promise.all([repository.jsonAPI_find(req),repository.count()]);
      res.json( new BananeSerializer(req,total).serialize(bananes)  );
    }catch(e) {
      next(e);
    }
  }
  ...
  public async remove (req: Request, res : Response, next: Function) {
    ...
    try {
      ...
      const banane = await bananeRepository.findOne(req.params.id);
      const repository = getRepository(Banane);
