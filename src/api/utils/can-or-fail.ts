...
export async function canOrFail<T> (authorizer: ResourceAuthorizer<T>, actor: UserModel, action: AuthorizerAction, on: T | T[], inContext: unknown) {
  const targets = Array.isArray(on) ? on : [on];
  await Promise.all(targets.map(async (target) => {
    if (!await authorizer.can(actor, action, target, inContext)) {
